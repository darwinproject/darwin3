...
.. math::
   ...
   \partial_t\op{DIC}   &= -\sum_j U^{\op{DIC}}_j \cdot (1 + R^{\text{PIC:POC}}_j)
                           + \sum_j R^{\op{DIC}}_j + R_{\op{DOC}} + [R_{\op{POC}}]
                           + \kappa^{{\text{diss}}}_{\mathrm{C}}\op{PIC}
                           + R^{\mathrm{C}}_j \\
   \partial_t\op{PO}_4  &= -\sum_j U^{\op{PO4}}_j + \sum_j R^{\op{PO4}}_j + R_{\op{DOP}} + [R_{\op{POP}}] \\
   \partial_t\op{NH}_4  &= -\sum_j U^{\op{NH4}}_j + \sum_j R^{\op{NH4}}_j + R_{\op{DON}} + [R_{\op{PON}}]
                           - P_{\op{NO2}} - D_{\op{NH4}} \\
   \partial_t\op{NO}_2  &= -\sum_j U^{\op{NO2}}_j + \sum_j R^{\op{NO2}}_j + P_{\op{NO2}} - P_{\op{NO3}} \\
   \partial_t\op{NO}_3  &= -\sum_j U^{\op{NO3}}_j + \sum_j R^{\op{NO3}}_j + P_{\op{NO3}} - D_{\op{NO3}} \\
   \partial_t\op{SiO}_2 &= -\sum_j U^{\op{SiO2}}_j + R_{\op{POSi}} \\
   \partial_t\op{FeT}   &= -\sum_j U^{\op{Fe}}_j + \sum_j R^{\op{FeT}}_j + R_{\op{DOFe}} + [R_{\op{POFe}}] + S_{\op{Fe}} \\
